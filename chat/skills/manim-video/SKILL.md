...
## Tool Integration: write_study_video

This skill is loaded via the `read_skill` tool when generating videos with `write_study_video`.

### Script Requirements for write_study_video

```python
from manim import *

class MyVideoName(Scene):  # or ThreeDScene for 3D
    def construct(self):
        self.camera.background_color = "#1C1C1C"  # dark hex
        # ... animations ...
```

**Mandatory:**
- `from manim import *` only — no `manim_slides`
- Class extends `Scene` (2D) or `ThreeDScene` (3D)
- `self.camera.background_color = "#1C1C1C"` (or dark hex)
- Clean exit: `self.play(FadeOut(Group(*self.mobjects)))`

### 2D Patterns (Scene)
- `Text`, `MathTex`, `Create`, `Write`, `FadeIn`, `FadeOut`, `Transform`
- `VGroup` for grouping, `Axes` for plots
- `self.wait(N)` for pacing

### 3D Patterns (ThreeDScene)
- `self.set_camera_orientation(phi=75*DEGREES, theta=30*DEGREES)`
- `ThreeDAxes()`, `Surface()`, `Sphere()`, `ParametricSurface()`
- `self.move_camera(phi, theta, run_time=2)` for camera animation
- Works with Cairo renderer — **no OpenGL required**

### Avoid (will crash)
- ❌ `Slide` from `manim_slides` — use `Scene`/`ThreeDScene`
- ❌ `renderer="opengl"` — server uses Cairo (Cairo 3D works)

### Quality & Output
- Tool uses `-ql` (480p15) for speed; override in skill if needed
- Output: self-contained HTML with embedded base64 MP4
- LaTeX via `MathTex` works (MiKTeX on server)
